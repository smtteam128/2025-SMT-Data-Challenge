...
players <- plays_sprints %>%
  group_by(player_code, sprint_time_95th) %>%
...
#Player responsibility score - RRS + Agg Index
responsibility_scores <- plays_sprints %>%
  group_by(game_str, play_id) %>%
  filter(n() == 3) %>%
  mutate(raw_responsibility_score = round(adjusted_prob/(sum(adjusted_prob))*adjusted_prob, 4)) %>%
  mutate(aggressivenss_index = attempted_catch - raw_responsibility_score) %>%
  ungroup()

colnames(responsibility_scores)

#If the player catching the ball was the only player with a non-zero chance to catch the ball
responsibility_scores$aggressivenss_index <- ifelse(responsibility_scores$adjusted_prob == responsibility_scores$raw_responsibility_score & responsibility_scores$attempted_catch == 1, 0, responsibility_scores$aggressivenss_index)
#We dont care about plays less than 1 percent 
responsibility_scores$aggressivenss_index <- ifelse(responsibility_scores$adjusted_prob < 0.01, 0, responsibility_scores$aggressivenss_index)

#Aggregate to the player level
player_agg <- responsibility_scores %>%
  group_by(player_code) %>%
  summarize(agg_index = 100*(sum(aggressivenss_index)/n()), total = n())

#Standardize it on a scale of 0 to 100
player_agg$agg_index <- (rank(player_agg$agg_index) - 1) / (length(player_agg$agg_index) - 1) * 100
#Look at team stats------------------------------------------------

#Get all the players involved for each play
player_trios <- responsibility_scores %>%
  group_by(game_str, play_id) %>%
  summarise(
    players = list(sort(player_code)),
    .groups = "drop"
  ) %>%
  mutate(
    player_1 = map_chr(players, ~.x[1]),
    player_2 = map_chr(players, ~.x[2]), 
    player_3 = map_chr(players, ~.x[3])
  )

#Now group by the player columns
grouped_by_trio <- player_trios %>%
  group_by(player_1, player_2, player_3) %>%
  summarise(
    num_plays = length(play_id),
    .groups = "drop"
  )

#Set a base for the minimum number of plays
grouped_by_trio <- filter(grouped_by_trio, num_plays > 25)

#join with aggressivenes score
trio_stats <- left_join(grouped_by_trio, player_agg, by = c("player_1" = "player_code"))
trio_stats <- left_join(trio_stats, player_agg, by = c("player_2" = "player_code"))
trio_stats <- left_join(trio_stats, player_agg, by = c("player_3" = "player_code"))

trio_stats <- select(trio_stats, player_1, p1_agg_index = agg_index.x, player_2,
                     p2_agg_index = agg_index.y, player_3, p3_agg_index = agg_index, num_plays)

#Calculate the volatility score by taking the standard deviation
trio_stats$volatility_score <- apply(trio_stats[, c("p1_agg_index", "p2_agg_index", "p3_agg_index")], 
                                    1, sd, na.rm = TRUE)

#Calculate the aggressiveness score by taking the mean
trio_stats$aggressiveness_score <- apply(trio_stats[, c("p1_agg_index", "p2_agg_index", "p3_agg_index")], 
                                     1, mean, na.rm = TRUE)

#Save models and values
saveRDS(naive_rf, "random_forest_model1.rds")
saveRDS(gam_model, "gam_model1.rds")
write.csv(plays_sprints, "final_dataset2.csv")
write.csv(player_responsibility_score, "player_responsibility_score.csv")
write.csv(responsibility_scores, "responsibility_score.csv")
