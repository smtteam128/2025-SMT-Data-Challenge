...
responsibility_scores <- filter(responsibility_scores, highest_prob_on_play > 0.5)
#Aggregate to the player level
player_responsibility_score <- responsibility_scores %>%
  group_by(player_code) %>%
  summarize(agg_index = sum(aggressivenss_index), oaa = sum(is_out - naive_catch_prob), total = n())

#Save models and values
saveRDS(naive_rf, "random_forest_model1.rds")
saveRDS(gam_model, "gam_model1.rds")
write.csv(plays_sprints, "final_dataset1.csv")
